...
## Hard Rule: Case names must directly express test intent

A case name must let a reviewer understand from the title alone:
- what business fact the case proves, or
- what business risk the case prevents

Do not write case names as checklist-style labels such as "test object + check/validation/verification".

Bad examples:
- Cancel validation
- GTC order check
- Partial fill replace verification

Good examples:
- A terminal order must reject a second cancel request and must not revive the old order
- A GTC order must remain valid across day cut and must not be cleaned up by mistake
- After a partial fill, the replace quantity must not be lower than the cumulative filled quantity
...
**Documentation rule:** The written requirements analysis must make the **cross-source reconciliation conclusion** visible to reviewers (not only in chat). Same for the test design doc: case preconditions and expected results must be consistent with the **documented** baseline, not an unstated pick among sources.
...
The case `name` must convey the test intent — a reader should understand what business fact the case verifies from the name alone, not just the sequence of actions performed.
...
## Complex-system reminder (multi-layer chains / state windows)

When the system under test is a **multi-layer chain** (e.g. `gateway -> router -> downstream dependency`) or has **time/state windows** (maintenance windows, switch-over windows, cycle boundaries), cross-source reconciliation should routinely compare at least:
- External specs / public docs vs internal implementation docs (field semantics, timezone, phase names, revision dates consistency)
- External-facing interface contracts vs internal component behavior (do all layers agree on the same event semantics)

If the current task is this kind of complex business, build an explicit **rule matrix** before writing cases.
Fill in every cell, then verify each "reject" cell maps to at least one failure case,
and each "allow but pass-through" cell maps to at least one case that distinguishes
**local reject** from **downstream reject**.

Example matrix (`<window>` × `<request type>`, fill each cell with expected behavior):

| Window / state | Request type A | Request type B |
|--------|-------------|------------|
| Normal | ? | ? |
| Boundary (pre switch-over) | ? | ? |
| Boundary (post switch-over) | ? | ? |
| Maintenance | ? | ? |

Also inspect whether these generic dimensions need coverage:
- differences between request paths
- pass-through semantics (downstream request sent + result forwarded unchanged)
- externally/downstream-driven terminal events (no prior client-initiated request required)
- customer/tenant-type split and their boundary-period behavior difference
- connection failure vs downstream rejection vs local reject — all three must be distinguishable
- cycle switch-over or operational restart paths and directory/date correctness
- business-date / processing-date consistency across layers
- clock or calendar drift between upstream and downstream components
- delayed message crossing session boundary (ingress time vs downstream send time mismatch)
